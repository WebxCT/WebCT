...
class SCINTILLATOR_MATERIAL(str, Enum):
	...
	@staticmethod
	def from_str(material: str):
		return SCINTILLATOR_MATERIAL(
			{
				"CUSTOM": "CUSTOM",
				"CSI": "CsI",
				"NaI": "NaI",
				"Gadox": "Gadox",
				"Gadox DRZ-Plus": "Gadox DRZ-Plus",
				"Gd2O2S DRZ-Plus": "Gadox DRZ-Plus",
				"Gd2O3": "Gd2O3",
				"Gd3Ga5O12": "Gd3Ga5O12",
				"YGO": "YGO",
				"CdWO4": "CdWO4",
...
class Scintillator:
	...
	def response(self) -> EnergyResponse:
		if self.material != SCINTILLATOR_MATERIAL.NONE:
			...
			return EnergyResponse(
				incident=tuple([x[0] for x in incident_output]), output=tuple([x[1] for x in incident_output])
			)
		else:
	...
	def from_json(json: dict):
		if "material" not in json or "thickness" not in json:
		...
		material = SCINTILLATOR_MATERIAL.from_str(str(json["material"]))
		...
		if material == SCINTILLATOR_MATERIAL.CUSTOM:
			...
			custom_response = EnergyResponse(incident=incident, output=output)

		return Scintillator(material=material, thickness=thickness, custom_response=custom_response)
...
class DetectorParameters:
	...
	@staticmethod
	def from_json(json: dict):
		if "pane_width" not in json or "pane_height" not in json or "pixel_size" not in json:
			raise ValueError("Missing keys.")
		...
		# Gain
		enableGain = bool(json["enableGain"])
